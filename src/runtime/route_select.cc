...
// SIMD LinearScan was explored on this branch (commits 7612ec2,
// "per-route SIMD"; later, "cross-route SoA SSE2") and abandoned.
// Both approaches lost 5-15× to scalar LinearScan in bench because
// the scalar inner loop's first-byte-mismatch early-exit checks
// just ~1 byte per non-matching route — SIMD setup cost (load XMM,
// cmpeq, movemask, branch) is heavier than 1 byte compare. The
// `caps` parameter is kept on pick_dispatch as forward-compatible
// API for any future SIMD-enabled dispatch (e.g., ART's Node16
// PCMPEQB), but no current dispatch consults it.
...
const RouteDispatch* pick_dispatch(const RouteAnalysis& a, const CpuCaps& caps) {
    // caps is plumbed through but no current dispatch is SIMD-gated.
    // See block comment near kLinearScanCutoff for why SIMD-LS was
    // tried and abandoned on this branch.
    (void)caps;
